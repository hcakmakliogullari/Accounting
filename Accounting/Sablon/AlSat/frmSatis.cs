        ...
        private void YeniKaydet()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Liste.RowCount; i++)
                {
                    ...
                    {
                        for (int a = 0; a < Liste.ColumnCount; a++)
                        {
                ...
                else
                {
                    if (kon)
                    {
                        MessageBox.Show("Firma, Personel ve Kargo alanı boş bırakılamaz.");
                    }
                }
            ...
            try
            {
                ...
                if (e.ColumnIndex == 1)
                {
                    ...
                    var lts = (from s in _db.tblStocks where s.ProductID == int.Parse(Liste.CurrentRow.Cells[0].Value.ToString()) && s.Quantity > 0 select s).ToList();
                     comboCell = (DataGridViewComboBoxCell)Liste.CurrentRow.Cells[2];
